...
@Component({
  ...
  styleUrls: ['./beneficio-detalle.component.css']
})
export class BeneficioDetalleComponent implements OnInit {
  public benefitId: any;
  public url: string = '';
  ...
  getBeneficioData(){
    this.url = `benefits/${this.benefitId}`;
...
upDateImage(img: any) {
  let url = `benefits/${this.benefitId}/image`;
  ...
  this.abstractSv.put(url, body, header).subscribe({
    ...
    complete: () => this.getBeneficioData()
  })
}
...
getData(datos?:any){
  ...
  this.orderName = this.datos.order
    let url = this.createUrl(this.datos);
    console.log(url)
    this.abstractSv.get(url).subscribe((res: any)=> {
      this.items = res;
...
guardarCambios(data: any) {
  console.log(data)
  if (!data) return;
  let url = `benefits/${this.benefitId}`;
  let header = { 'x-content-type': 'on' };
  let body = { storeId: data.id };
  
    this.abstractSv.put(url, body, header).subscribe({
      next: (res) => this.getBeneficioData(),
      error: (err) => console.log(err),
